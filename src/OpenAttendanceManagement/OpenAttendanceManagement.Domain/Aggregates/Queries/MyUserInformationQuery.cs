...
public record MyUserInformationQuery(TenantCode TenantCode)
    : ITenantNextGeneralQueryAsync<MyUserInformationQuery, MyUserInformationQuery.Result>
{
    public string GetTenantId() => TenantCode.Value;
    public static Task<ResultBox<Result>> HandleFilterAsync(MyUserInformationQuery query, IQueryContext context) =>
        context
            .ExecuteQueryAsync(new TenantFromCodeQuery(query.TenantCode))
            ...
            .Conveyor(
                information => information
                    ...
                    .Match(
                        userId => context
                            .GetAggregateState<OamTenantUser>(userId.Value, query.TenantCode.Value)
                            .Conveyor(
                                user => ResultBox.FromValue(
                                    new Result(
                                        OptionalValue.FromValue(
                                            new ExistingUserResult(
                                                userId.Value,
                                                user.Payload.UserName,
                                                user.Payload.DisplayName,
                                                information.AuthIdentityEmail)),
                                        information.GetType().Name))),
                        ResultBox.FromValue(
                            new Result(
                                OptionalValue<ExistingUserResult>.Empty,
                                information.GetType().Name)))
            );
