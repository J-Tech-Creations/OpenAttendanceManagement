...
public static class AppContextSeed
{
    ...
    public static ResultBox<UnitValue> CreateRoleIfNotExist(string name, AuthDbContext context)
        => ResultBox.Start
            .Conveyor(
                _ => context.Roles.Any(m => m.NormalizedName == name.ToUpper()) switch
                {
            ...
            .ConveyorWrapTry(
                _ => context.Roles.Add(
                    new IdentityRole { Name = name, NormalizedName = name.ToUpper() }))
            .Scan(_ => context.SaveChanges())
            .Remap(_ => UnitValue.Unit);

    public static ResultBox<UnitValue> CreateUserRoleFor(
        string email,
        string roleName,
        ...
        => ResultBox.Start
            ...
            .Verify(
                values => context.UserRoles.Any(
                        ur => ur.UserId == values.Value1.Id && ur.RoleId == values.Value2.Id) switch
                    {
